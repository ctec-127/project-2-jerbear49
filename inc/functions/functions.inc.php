...
function display_record_table($records)
{
    ...
    foreach ($records as $row) {
        ...
        echo "<td class=\"text-center{$class}\"><strong>{$row->gpa}</strong></td>";
        if ($row->financial_aid == 1) {
            $fin_aid_text = "✅";
        } else if ($row->financial_aid == 0) {
            $fin_aid_text = "";
        }
        echo "<td class=\"text-center\"><strong>{$fin_aid_text}</strong></td>";
        echo "<td><strong>{$row->email}</strong></td>";
        echo "<td><strong>{$row->phone}</strong></td>";

        // If $graduation_date is not null, format and display it.
        if ($row->graduation_date != null) {
            $date = date_format(date_create($row->graduation_date), "m/d/Y");
            echo "<td class=\"text-center\"><strong>{$date}</strong></td>";
        } else {
            echo "<td></td>";
        }
        echo '</tr>';
